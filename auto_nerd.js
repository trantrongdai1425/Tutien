// AUTO NERD - Normalized Script for Browser Console (F12)

(function () {
	var sleep = function (ms, maxJitter) {
		// Tự động tạo độ lệch ngẫu nhiên nhẹ (+0ms đến +200ms) để chống phát hiện Anti-bot
		// nhưng vẫn giữ tốc độ auto mượt mà, không bị chậm
		var jitterRange =
			maxJitter !== undefined
				? maxJitter
  ...
  `;
	...
	function clearHealEphemeralNotice() {
		...
		btns.forEach(function (btn) {
			...
			// 1. TUYỆT ĐỐI BẢO VỆ: Nếu là giao diện Lịch Luyện, Boss hoặc Bí Cảnh thì BỎ QUA NGAY!
			var isProtectedUI =
			...
			// 2. CHỈ bấm Bỏ qua nếu ĐÚNG LÀ TIN NHẮN THÔNG BÁO HỒI PHỤC THUỐC
			var isHealNotice =
	...
	function checkTeamStamina(scope) {
		...
		if (memberItems && memberItems.length > 0) {
			...
			// Fallback quét theo dòng text của container hiện tại
			var lines = txt.split("\n");
	...
	function findHomeBiCanhButton() {
		...
		// 1. Quét tối đa 8 tin nhắn gần nhất ngược về cũ để tìm menu Trang Chủ / Dashboard
		for (var i = msgs.length - 1; i >= endIdx; i--) {
			var m = msgs[i];
			var mTxt = (m.innerText || m.textContent || "").toLowerCase();
			// Nhận diện tin nhắn trang chủ dựa trên các mục: "Tu Luyện", "Thông Tin", "Hồ Sơ", "Động Phủ", "Lịch Luyện"
			if (
		...
		// 2. Fallback: Quét trong 8 tin nhắn gần nhất (không phải tin nhắn embed phòng hiện tại) có nút "Bí Cảnh"
	...
	function runVanMoc() {
		...
		async function loop() {
			...
			if (!scope) return; // Chưa có tin nhắn/embed game -> Đợi tick sau
			...
			// 1. KẾT QUẢ NHANH KHI ĐANG ĐÁNH TRẬN
			...
			// 2. CHIẾN TIẾP (HOÀN THÀNH 1 LẦN ĐI BÍ CẢNH MỚI)
			...
			// 2.1 TIẾP TỤC (SANG ẢI PHỤ / ĐI TIẾP ẢI ĐANG DỞ)
			...
			// 3. KHAI CHIẾN / KHIÊU CHIẾN / CHIẾN ĐẤU (Mở rộng từ khóa, có chống spam debounce)
			var combatKeywords = [
				"khai chiến",
				"khiêu chiến",
				"chiến đấu",
			...
			// 4. SỰ KIỆN VƯỜN LINH NẤM HUYỀN BÍ (Random 1/3 nút)
			...
			if (mode === "SAFE" && btnCayThanA) {
				...
				reasonCayThan = "Chế độ An Toàn (*) -> Nuôi Dưỡng Cây Thần";
			} else if (mode === "RISK" && btnCayThanB) {
				chosenCayThan = btnCayThanB;
				reasonCayThan = "Chế độ Rủi Ro (**) -> Hái Hạt Giống Thần Mộc";
			}
			...
			if (mode === "SAFE" && btnLinhThuA) {
				...
				reasonLinhThu = "Chế độ An Toàn (*) -> Tha Mạng & Chữa Trị";
			} else if (mode === "RISK" && btnLinhThuB) {
				chosenLinhThu = btnLinhThuB;
				reasonLinhThu = "Chế độ Rủi Ro (**) -> Trảm Sát Thu Linh Vật";
			}
			...
			// 9. NÚT LÀM MỚI NẾU SẢNH MẤT NÚT
	...
	function runTayTuy() {
		...
		function clearTayTuyNoticeEphemerals() {
			...
			btns.forEach(function (btn) {
				...
				if (msg) {
					var txt = (msg.innerText || "").toLowerCase();
					// CHỈ xóa tin nhắn thông báo "Đạo hữu đã chọn giữ lại thể chất cũ..."
					var isNotice =
		...
		async function loop() {
			...
			// Tự động xóa thông báo rác "Đạo hữu đã chọn giữ lại thể chất cũ..."
			...
			// 1. KIỂM TRA TRANG KẾT QUẢ TẨY TỦY (Nút "Nhận Mới: ..." và "Giữ Cũ: ...")
			...
			if (btnNhanMoi || btnGiuCu) {
				...
				// Kiểm tra tên thể chất mới xem có chứa chữ "Thánh" hay không (không phân biệt hoa/thường)
			...
			// 2. KIỂM TRA NÚT "Tẩy Tủy (10,000 LT)" / "Tẩy Tủy"
	...
	function runBossSan() {
		...
		function isBossLobbyContainer(container) {
			...
			// 1. Từ khóa cấu trúc dùng chung cho TẤT CẢ các loại Boss (Trạm Lư, Tạ Diệm, v.v.):
			var hasBossKeywords =
			...
			// 2. Kiểm tra bộ nút bấm đặc trưng:
			var btns = Array.from(
		...
		function getBossScope() {
			...
			var targetEmbed = null;
			// Quét ngược 5 embed gần nhất từ dưới lên
			for (var i = embeds.length - 1; i >= 0 && i >= embeds.length - 5; i--) {
				var emb = embeds[i];
				var txt = (emb.innerText || "").toLowerCase();
				if (
		...
		function clearBossHealEphemerals() {
			...
			btns.forEach(function (btn) {
				...
				if (msg) {
					...
					// 1. CHỈ nhận diện tin nhắn chứa thông báo kết quả hồi máu / đếm hồi máu
					var isHealNotice =
					...
					// 2. BẢO VỆ SẢNH BOSS: Container không được là Sảnh Boss chính
					var isLobby = isBossLobbyContainer(msg);

					// CHỈ bấm Xóa tin nhắn khi đúng là tin nhắn thông báo hồi máu VÀ không phải Sảnh Boss
		...
		async function loop() {
			...
			// Tự động xóa thông báo rác "Hồi máu thành công" sau khi bấm Hồi Máu
			...
			var btns = Array.from(
				...
				if (b.disabled || !b.innerText) return false;
				var t = b.innerText.trim().toLowerCase();
				// TUYỆT ĐỐI KHÔNG BAO GIỜ LẤY "Bỏ qua tin nhắn", "Dismiss message" hoặc "Rời Sảnh"
				if (
					t.includes("bỏ qua tin") ||
					t.includes("dismiss") ||
					t.includes("rời sảnh") ||
			...
			// 0. ĐIỂM NGẮT CHƯƠNG TRÌNH: KHI BOSS BỊ ĐÁNH BẠI HOẶC CHƯA XUẤT HIỆN
			...
			// PHASE 1: TRANG TRUNG GIAN KHI BẤM ĐÁNH BOSS (Nút Sảnh Boss / Bỏ qua animation)
			...
			// PHASE 2: TRANG GIAO DIỆN SẢNH BOSS CHÍNH
			...
			// Step 1: Hồi máu trước nếu chưa hồi cho lượt này
			...
			// Step 2: Bấm nút đánh Boss (khi đã Hồi Máu VÀ đã HẾT CD 30s)
	...
	function runKimCuong() {
		...
		async function loop() {
			...
			// 1. KẾT QUẢ NHANH KHI ĐANG TRONG TRẬN
			var btnKetQua = btns.find(function (b) {
				var t = b.innerText.trim().toLowerCase();
			...
			// 3. KHAI CHIẾN / KHIÊU CHIẾN / CHIẾN ĐẤU (Có chống spam debounce)
			var combatKeywords = [
				"khai chiến",
				"khiêu chiến",
				"chiến đấu",
			...
			// 4. KIỂM TRA SỰ KIỆN ĐẶC BIỆT CHỈ CÓ 1 NÚT (HOẶC WIP)
			...
			// 5. KIỂM TRA CÁC KỲ NGỘ 2 LỰA CHỌN (OPTION A vs OPTION B)
			...
			for (var k = 0; k < kyNgoEvents.length; k++) {
				...
				if (btnA || btnB) {
					...
					if (useOptionB && btnB) {
						chosenBtn = btnB;
						choiceReason = hpInfo.isCritical
							? "MÁU NGUY KỊCH (" +
								hpInfo.lowHpCount +
								"/" +
								hpInfo.total +
								" người <20%) -> Bấm Option B (An Toàn/Hồi HP)"
							: "ĐÃ MAX STACK (3/3 Tầng) -> Bấm Option B (An Toàn/Hồi HP)";
					} else if (btnA) {
						chosenBtn = btnA;
						choiceReason =
							"MÁU KHỎE & CHƯA MAX STACK (" +
							stacks +
							"/3 Tầng) -> Bấm Option A (Lấy Stack *)";
					} else if (btnB) {
						chosenBtn = btnB;
						choiceReason = "Không tìm thấy nút Option A -> Bấm Option B";
					}
			...
			// 6. ĐIỀU HƯỚNG BÁT MÔN AN TOÀN CHUẨN
			var targetGate = null;
			var pList = ["sinh", "hưu", "cảnh", "khai", "thương", "kinh"];
			for (var i = 0; i < pList.length; i++) {
				var p = pList[i];
			...
			// 8. NÚT LÀM MỚI NẾU SẢNH MẤT NÚT
			var btnRefresh = btns.find(function (b) {
				var t = b.innerText.trim().toLowerCase();
				return (
					t.includes("làm mới") ||
	...
	function runStandardBC(config) {
		...
		async function loop() {
			...
			// 4. KIỂM TRA SỰ KIỆN PHỤ BÍ CẢNH THƯỜNG
			var extraEvents = [
				"rời đi giữ mình",
				"tránh xa hố đen",
			...
			// 7. NÚT LÀM MỚI NẾU SẢNH MẤT NÚT
			var btnRefresh = btns.find(function (b) {
				var t = b.innerText.trim().toLowerCase();
				return (
					t.includes("làm mới") ||
	...
	function runUMinh() {
		...
		var healMode = false;
		var isHealing = false;
		var mode = "BALANCE"; // "SAFE", "BALANCE", or "RISK"
		var startClickCount = 0;
		var lastActionName = "";
		var actionClickCount = 0;
		var noButtonTickCount = 0;
		...
		async function loop() {
			...
			// 4. KIỂM TRA 6 KỲ NGỘ U MINH THEO CHẾ ĐỘ (GIỮ NGUYÊN 100% LOGIC ĐỘC QUYỀN)
			for (var i = 0; i < uMinhEvents.length; i++) {
				...
				if (ev.name === "Quan Tài Bí Ẩn" || ev.name === "Tà Đàn Hóa Ma") {
					...
					if (mode === "SAFE" || mode === "BALANCE") {
						...
						choiceDesc = "Chế độ Rủi Ro / Boss Ẩn (**/***)";
					}
				}
	...
	function runLich10() {
		...
		function checkError() {
			...
			// 1. Kiểm tra số thể lực thực tế hiện có (số X nằm trước dấu / trong "Hiện có: X/400")
			...
			// 2. Kiểm tra nếu nút "Nhanh x10" trong message mới nhất bị disabled
		...
		async function loop() {
			...
			// 1. KIỂM TRA MÁU DƯỚI 90% ĐỂ HỒI MÁU TỰ ĐỘNG
			if (isHealMode10) {
	...
	function runLich() {
		...
		function checkError() {
			...
			// 1. Đọc số Thể lực hiện có (số X nằm trước dấu / trong "Hiện có: X/400")
			var stamina = getCurrentStamina(scope);
			...
			// 2. Kiểm tra nếu nút "Chiến Lại" trong message mới nhất bị disabled
	...
	function runLamVuon() {
		...
		function getGardenScope() {
			...
			var targetEmbed = null;
			// Quét ngược 5 tin nhắn gần nhất từ dưới lên (mới nhất -> cũ hơn)
			for (var i = embeds.length - 1; i >= 0 && i >= embeds.length - 5; i--) {
				var emb = embeds[i];
				var txt = (emb.innerText || "").toLowerCase();
				if (
		...
		async function loop() {
			...
			if (page === 1) {
				...
				// Nếu giếng hết CD nhưng chưa bấm được -> Bấm Làm Mới ở Trang 1
				if (wellCDUntil <= now) {
			...
			if (page === 3) {
				...
				// 0. TỰ ĐỘNG THU HOẠCH AOE & DỪNG KHI CÂY CHÍN MUỒI >= 80%
